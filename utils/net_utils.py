...
def printModelScore(model, args):
    for n, m in model.named_modules():
        if hasattr(m, "scores"):
            ...
            print("grad info: mean: %.6f, variance: %.6f, max: %.4f, min: %.6f" % (torch.mean(m.scores.grad).item(), torch.var(m.scores.grad).item(), torch.max(m.scores.grad).item(), torch.min(m.scores.grad).item()))
